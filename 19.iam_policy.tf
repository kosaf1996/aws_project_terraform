...
resource "aws_iam_policy" "Seoul-AccessingECRRepositoryPolicy" {
  name        = "Seoul-AccessingECRRepositoryPolicy"
...
resource "aws_iam_policy" "Tokyo-AccessingECRRepositoryPolicy" {
  name        = "Tokyo-AccessingECRRepositoryPolicy"
  description = "Codebulid가 ECR에 접근하기 위한 정책"
    ...
    "Statement" : [
      {
        ...
        "Resource" : [
          "arn:aws:ecr:ap-northeast-1:${local.account_id}:repository/${var.django_ecr}"
        ]
      },
      {
        "Sid" : "GetAuthorizationToken",
...
# ECS Task Definition 생성 시 자동으로 생성되는 Role에 연결할 AWS 관리형 정책
